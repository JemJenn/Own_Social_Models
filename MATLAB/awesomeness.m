...
p1c(n1c,n2c,n1,beta1,beta2,alpha1,alpha2,b,c)= n1*( n1c*(b-c)  +  n1d*(-c) )+(n2)*( n2c*beta1*beta2*alpha1*n1*(b-c)  +  n2c*beta2*(1-beta1)*b  +  n2c*beta1*(1-beta2)*(-c) + n2d*beta1*(-c));
p1d(n1c,n2c,n1,beta1,beta2,alpha1,alpha2,b,c)= n1*( n1c*b ) + (n2)*( n2c*beta2*b );
p2c(n1c,n2c,n1,beta1,beta2,alpha1,alpha2,b,c) = (n2)*( n2c*(b-c) + n2d*(-c) ) + (n1)*( n1c*beta1*beta2*alpha2*n2*(b-c) + n1c*(1-beta1)*beta2*(-c) + n1c*(1-beta2)*beta1*(b) + n1d*beta2*(-c));
...
n1cnull(n2c,n1,beta1,beta2,alpha1,alpha2,b,c)=solve(n1cdot==0,n1c)
n2cnull(n1c,n1,beta1,beta2,alpha1,alpha2,b,c)=solve(n2cdot==0,n2c)
n1null(n1c,n2c,beta1,beta2,alpha1,alpha2,b,c)=solve(n1dot==0,n1)
%for 1-d 
...
%     vec = subs(vec,beta1,beta2);
%     lam = subs(lam,beta1,beta2);
%     vec = subs(vec, alpha1,alpha2);
%     lam = subs(lam,alpha1,alpha2);
    vec= simplify(vec)
    lam = simplify(lam)
end
