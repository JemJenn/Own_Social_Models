...
%T = 0.1; %separation of scales in replicator dynamics
    ...
    for alpha1 = 6.1:10:26.1
        for alpha2=1.1:10:20.1
            for beta1=0.6:.2:1
                for beta2 = 0.6:.2:1
                    
                    p1c(n1c,n2c,n1)= n1*( n1c*(b-c)  +  n1d*(-c) )+(n2)*( n2c*beta1*beta2*alpha1*(b-c)  +  n2c*beta2*(1-beta1)*b  +  n2c*beta1*(1-beta2)*(-c) + n2d*beta1*(-c));
                    p1d(n1c,n2c,n1)= n1*( n1c*b ) + (n2)*( n2c*beta2*b );
                    p2c(n1c,n2c,n1) = (n2)*( n2c*(b-c) + n2d*(-c) ) + (n1)*( n1c*beta1*beta2*alpha2*(b-c) + n1c*(1-beta1)*beta2*(-c) + n1c*(1-beta2)*beta1*(b) + n1d*beta2*(-c));
                    ...
                    for fp = 1:length(n1cstar)
                        fp
                        if real(double(n1cstar(fp)))== 1  &real( double(n2cstar(fp)))==1&real(double(n1star(fp)))<1 &real(double(n1star(fp)))>0
                            
                            lam = eig(shayni(n1cstar(fp),n2cstar(fp),n1star(fp)))
                            internal = [internal;[fp,lam(1),lam(2),lam(3)]]
